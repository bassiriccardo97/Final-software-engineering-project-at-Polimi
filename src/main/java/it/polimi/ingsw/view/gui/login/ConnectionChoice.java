...
public class ConnectionChoice {
    ...
    private Button rmi = new Button();
    private Button tcp = new Button();
    ...
    public ConnectionChoice() {

        rmi.setText("RMI");
        rmi.setTextAlignment(TextAlignment.CENTER);
        rmi.setContentDisplay(ContentDisplay.CENTER);
        rmi.setPrefWidth(50);
        rmi.setDefaultButton(true);
        rmi.addEventFilter(MouseEvent.MOUSE_CLICKED, rmiSelectedWithMouse);
        rmi.addEventFilter(KeyEvent.KEY_PRESSED, rmiSelectedWithEnter);

        tcp.setText("TCP");
        tcp.setTextAlignment(TextAlignment.CENTER);
        tcp.setContentDisplay(ContentDisplay.CENTER);
        tcp.setPrefWidth(50);
        tcp.addEventFilter(MouseEvent.MOUSE_CLICKED, tcpSelectedWithMouse);
        tcp.addEventFilter(KeyEvent.KEY_PRESSED, tcpSelectedWithEnter);
        ...
        GridPane.setConstraints(rmi, 1, 0);
        GridPane.setConstraints(tcp, 0, 0);
        GridPane.setHalignment(rmi, HPos.CENTER);
        GridPane.setHalignment(tcp, HPos.CENTER);
        gridPane.getChildren().addAll(rmi, tcp);
    ...
    private EventHandler<MouseEvent> rmiSelectedWithMouse = mouseEvent -> {
        MyApplication.getClient().setRmiConnection(true);
        MyApplication.getClient().setGuiInterface(true);
        MyApplication.getClient().startClientRMIforGUI();
        nextScene();
    };

    private EventHandler<MouseEvent> tcpSelectedWithMouse = mouseEvent -> {
        MyApplication.getClient().setRmiConnection(false);
        MyApplication.getClient().startClientSocketForGUI();
        MyApplication.getClient().setGuiInterface(true);
    };

    private EventHandler<KeyEvent> rmiSelectedWithEnter = keyEvent -> {
    ...
    private EventHandler<KeyEvent> tcpSelectedWithEnter = keyEvent -> {
        if (keyEvent.getCode() == KeyCode.ENTER) {
            MyApplication.getClient().setRmiConnection(false);
            MyApplication.getClient().setGuiInterface(true);
            MyApplication.getClient().startClientSocketForGUI();
